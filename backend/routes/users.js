...
router.post('/register', async (req, res) => {
  try {
    const { name, email, password, phoneNo, address, gender, role } = req.body;
    if (!name || !email || !password || !phoneNo || !address || !gender || !role)
    ...
    user = await User.create({ name, email, password, phoneNo, address, gender, role });
